...
/* ---- portfolio n3 : detail d'une UE au clic ---- */
(function () {
  "use strict";
  var modal = document.getElementById("uem");
  if (!modal) return;
  var box = modal.querySelector(".uem-body"),
      ttl = modal.querySelector(".uem-title"),
      sem = modal.querySelector(".uem-sem"),
      last = null;

  function open(card) {
    var tpl = document.getElementById(card.getAttribute("data-ue"));
    if (!tpl) return;
    last = card;
    ttl.textContent = tpl.getAttribute("data-titre") || "";
    sem.textContent = (tpl.getAttribute("data-sem") || "") +
                      " · " + (tpl.getAttribute("data-comp") || "");
    box.innerHTML = "";
    box.appendChild(tpl.content.cloneNode(true));
    modal.hidden = false;
    document.body.style.overflow = "hidden";
    modal.querySelector(".uem-x").focus();
  }
  function close() {
    modal.hidden = true;
    box.innerHTML = "";
  ...
  document.querySelectorAll(".uecard").forEach(function (c) {
    c.addEventListener("click", function () { open(c); });
    c.addEventListener("keydown", function (ev) {
      if (ev.key === "Enter" || ev.key === " ") { ev.preventDefault(); open(c); }
    });
  });
